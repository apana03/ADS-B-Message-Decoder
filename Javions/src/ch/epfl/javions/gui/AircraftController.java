...
public final class AircraftController {
    ...
    private void drawIcon(ObservableAircraftState state, GeoPos position, double altitude) {
        ...
        iconPath.layoutXProperty().bind(Bindings.createDoubleBinding(() ->
                WebMercator.x(mapParameters.getZoomValue(), position.longitude()) - mapParameters.getMinXValue())
        );
        iconPath.layoutYProperty().bind(Bindings.createDoubleBinding(() ->
                WebMercator.y(mapParameters.getZoomValue(), position.latitude()) - mapParameters.getMinYValue())
        );
        iconPath.viewOrderProperty().bind(state.altitudeProperty().negate());
    }
